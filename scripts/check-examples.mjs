 ...
 *
 * main - see frameworkRef in lib/release.mjs for why this used to be the
 * release and no longer is. In short: releases are monthly, merges are daily,
 * and a page that cannot be corrected until the next tag is a page that
 * teaches the old API for weeks. The sample corpora already compile against
...
console.log(REF
  ? `             compiled against abap2UI5 ${REF}${process.env.A2UI5_REF ? ' (A2UI5_REF)' : ' — the framework branch this site tracks'}`
