...
| `docs/.vitepress/theme/cost-calculator.js` | The cost calculator (`resources/cost_calculator`): sliders whose readouts follow them, a choice of support, and a Calculate button that greys the inputs and hands out the sheet - which echoes the readouts, in the currency chosen - until the next round. What the sheet hands out beyond ten zeros is the point of the page: the links out (the issue tracker, Slack) sit on its Support line rather than in the input rows, and under the total come the six things the zeros get you. `theme/cost-model.js` is the half with no DOM in it - the reading of a position, and the total, which is zero for any input; `test/cost-calculator.test.mjs` pins the arithmetic and holds the page's written readouts to what the script would write |
