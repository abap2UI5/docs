...
The approach: build the HTML with `cl_demo_output=>get( )`, inject CSS styles as the content of a `core:HTML` control, and render the result with the UI5 `html` control. The CSS block is needed because `CL_DEMO_OUTPUT` produces HTML with specific class names (e.g., `heading1`, `header`, `body`) that need matching style definitions to render correctly.
