...
To share the current app state instead of a custom string, compose the link with `client->app_state_get_href( )` and hand it to the same `clipboard_copy` action — see [App State, Share, Bookmark](../expert_more/app_state_share.md).
