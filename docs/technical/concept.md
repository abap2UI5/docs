...
In the early days of web development, SSR was the standard. Every user interaction caused a full-page reload, and the server responded with a complete HTML document. In the SAP ecosystem, this approach appeared in technologies like ITS Mobile and SAP GUI for HTML.
...
Since UI5 is a client-side framework, the backend can't build ready-to-render HTML. Instead, the browser creates the HTML with JavaScript through the UI5 framework.

#### Sending Views from Backend

So how can we produce UI5 HTML in the backend?

Luckily, UI5 has a key trait that lets us shift part of the view creation to the backend. In UI5 freestyle apps, you define each view in XML — the UI5 XML View. The UI5 framework uses this XML definition, combined with data from the backend, to create HTML in the browser.
...
The frontend becomes a static shell shared across all applications. The backend fully defines and maintains views and logic, and each application lives entirely inside backend ABAP classes. The result: every UI5 project becomes a backend project — no separate frontend deployment needed:
...
This means the frontend no longer reads CDS Views and OData services directly. Instead, the backend sends the entire UI state — view and model — in a single response. Any user changes in the UI then return to the backend through a lightweight AJAX call with the updated view model — no OData routing needed.

You don't need to set up models or bindings manually — abap2UI5 handles this internally. Expose class attributes through a simple bind method, and abap2UI5 takes care of the rest.
...
The initial request delivers the static shell app. After each user interaction, the app calls the backend — in PAI/PBO fashion — to fetch the updated view and model. Frontend and backend stay tightly coupled — not through OData service definitions, but through plain ABAP logic and JSON. The result: a fully backend-driven UI flow.
...
In standard UI5, updating the XML View usually causes a full re-render. But abap2UI5 enables partial updates by updating only the view model. This lets UI5 refresh only the relevant UI controls through data binding — without recreating the entire view structure.
...
Thanks to UI5's data binding mechanism, UI5 updates only the changed DOM elements. This keeps the current UI state intact — like input focus — and delivers a smooth, uninterrupted user experience.
...
By moving UI control to the ABAP backend and using UI5 only for HTML rendering, abap2UI5 enables fast and efficient development of business applications — without SPA complexity.
