...
This leads to the second small shift in abap2UI5: instead of binding OData to the view, abap2UI5 uses a custom view model built entirely in the backend. The backend builds this model dynamically after each request — tailored to the current view — and sends it together with the view definition to the frontend:
