...
To support user interaction, the initial HTTP request delivers a minimal, static UI5 freestyle app. This app contains just enough logic to forward frontend events and usually acts as a shell application. The interaction model takes inspiration from the classic PAI/PBO pattern familiar from SAP GUI applications.
