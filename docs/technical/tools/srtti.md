...
In abap2UI5, you don't necessarily need to define your data model at design time. Instead, you can work with generic data references and apply typing dynamically at runtime based on your program logic. This helps especially with scenarios like tables, where different columns and table types may appear based on user input.

abap2UI5 serializes app instances so client communication stays stateless. However, SAP's standard transformation features are limited and don't support data references with local types created at runtime.
...
This approach delivers compatibility with dynamic types while keeping the transformation process robust. It runs in the background, independently of the two app implementations above.
