...
And Fiori Elements, with all its floorplans and templates, is straightforward and will keep getting updates. In the end, the wide range of UI5 use cases makes comparing the approaches hard — and we can't fully discuss them here. Let's turn to the framework's codebase in the final part of this article.
...
With this approach, dependencies stay limited to APIs and functions that are both cloud-released and available in earlier NetWeaver releases. To handle this, abap2UI5 uses SAP dependencies only when truly needed — for example, in GUID creation:
...
Clearly, creating methods compatible with both 'ABAP Cloud' and 'Standard ABAP' is much more complex. Fortunately, abap2UI5 needs only GUIDs as a dependency. But when building apps, keep this in mind (and whether it's practical remains to be seen). It does have one key advantage: abap2UI5 runs on ABAP 2305 and stays portable down to NetWeaver 7.02.
