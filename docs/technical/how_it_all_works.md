...
Also, we send not just the data but also the metadata (Data Model) with every request (7). This differs from classic OData communication, where the initial OData request carries the metadata to establish the model up front, and only data moves afterward. With this approach, we can send different models with every request:
...
Since all user apps are also in pure ABAP, we can now maintain and build everything in the backend. Duplicating, editing, renaming, or other refactoring takes just moments. The deployment process shrinks to activating an ABAP class, so we can create many apps quickly. For example, we built all the apps in the sample section fast, mostly by copy-pasting — which would have been impractical for separately built and deployed frontend apps. This is a major reduction in complexity and an advantage of all 'Over the Wire' apps, as we noted in (3).
...
UI5 evolves rapidly, and extra layers can quickly become outdated. With this approach, all future UI5 controls work in abap2UI5 automatically. But there's a potential downside: you have to deal with the complexity of the frontend UI5 API and learn the concepts of XML views and UI5 controls. Ultimately, it's a matter of personal preference: learn UI annotations, or learn the concepts of SAP UI5 directly.

##### 20. No Hiding of Complexity

But having no extra layer also means the framework doesn't necessarily abstract complexity away — unlike what other frameworks aim for. In abap2UI5, you send your XML-View directly to the frontend, and you're responsible for making sure it's valid and executable:
...
As we saw in (22), we can make apps very complex, but the opposite also works — we can make them very simple. One beautifully minimal approach uses `if_oo_adt_classrun`. By implementing a single method, we generate output with one click (`F9`). This is very efficient and inspired abap2UI5. A comparison of both approaches:
...
Also, Fiori Elements with all its floorplans & templates is straightforward and will keep receiving updates. Ultimately, the wide range of UI5 use cases makes comparing the approaches difficult — and we can't fully discuss them here. Let's turn to the framework's codebase in the final part of this article.

##### 25. System Footprint

We keep the system footprint minimal — abap2UI5 relies only on ABAP classes, with no CDS or RAP artifacts. Most of the code lives in user apps outside the framework (21) (22). Overall, the framework comes to about 2,300 lines of code spread over one HTTP-Handler, two interfaces, and one database table:
...
Long article short: inspired by "HTML Over the Wire" (1)(2)(3), we mixed UI and Data together (7) and created a "UI5 Over the Wire" approach by sending the XML-View from the server (6). We then used a single generic HTTP-Service for all apps (13), independent of the View and Data Model (12). This gives us great flexibility, letting us create Data Models (10) and Views (11) dynamically at runtime, which markedly reduces the number of backend artifacts.

Next, we explored several ideas on how the framework reduces its own complexity by avoiding frontend artifacts (17), removing extra customizing layers (19), and separating the view from the framework (21), as well as app-specific JS or HTML (22). The result is a pure source-code approach with only one database table, two interfaces, one class, and just 2,300 lines of code (25). We build it in a single code line (27), making it cloud- and on-premise-ready and downportable to old releases (28). Together with abapGit, you can build apps that run on nearly every release.
