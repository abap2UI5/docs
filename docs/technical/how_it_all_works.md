...
In apps, we can now use RTTI again, similar to how it was used with ALVs. This means there is no need to create separate apps for each model. In this demo, you can see an abap2UI5 app with a view that contains a table output whose type is created and modified at runtime (similar to SE16):
...
With this approach, we achieve a stateful-like PAI/PBO feeling similar to SAP GUI apps, even though we are still operating within the AJAX roundtrip logic. Furthermore, since every request can be made to a different application server, abap2UI5 is compatible with scalable cloud environments, ensuring future compatibility:
...
UI5 is evolving rapidly, and additional layers can quickly become outdated. With this approach, all UI5 controls released in the future will automatically be usable in abap2UI5. However, a potential downside is that we have to deal with the complexity of the frontend UI5 API and learn the concepts of XML views and UI5 controls. Ultimately, it comes down to personal preference: learn UI annotations, or learn the concepts of SAP UI5 directly.
...
This contrasts with RAP, where you benefit from well-documented and organized extra layers, but sometimes they have limited functionality. Take side effects, for example: in RAP, you are restricted to using the `+`, `-`, and `*` operators. In abap2UI5, you have to write JavaScript directly, which requires much more knowledge, but it provides the benefit of accessing the full expression binding functionality available on the frontend:
...
With any request, you can send your own JavaScript or Custom Controls to the frontend. The abap2UI5 framework just sends it as-is to the frontend. All upcoming requests can now use this JavaScript — for example, to use Custom Controls in their UI5 Views:
