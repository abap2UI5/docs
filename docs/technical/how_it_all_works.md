...
As we saw in (22), we can build apps very complex, but the opposite also works — we can build them very simple. One minimal yet elegant approach uses `if_oo_adt_classrun`. By implementing a single method, we generate output with one click (`F9`). This is highly efficient and inspired abap2UI5. A comparison of both approaches:
