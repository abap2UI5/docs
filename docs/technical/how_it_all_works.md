...
By contrast, SPAs define all routes and actions up front on the frontend, requiring a full rebuild for any modification. The illustration below compares MPAs, SPAs, and HDAs:
...
Frameworks like Phoenix LiveView (2018) and Laravel Livewire (2019) were among the first to adopt this principle. Tools like htmx, hotwire, and unpoly followed, aiming to reduce complexity while preserving high UI fidelity. These frameworks seek a "sweet spot" between SPA and MPA architectures:
...
Despite still relying on frontend HTML rendering, the app now fetches all the needed information (view & data) over AJAX from the backend. The UI5 app remains a SPA, but its role shrinks to that of an HDA — responsible only for displaying the view and its data:
...
But when major Model and View changes are needed at runtime, this approach can be too inflexible. Model changes with RTTI aren't supported, and extending the view quickly exceeds the functional scope of backend annotations — requiring Fiori Elements apps (which again require extra deployment).
...
Since all user apps are also in pure ABAP, we can now maintain and build everything in the backend. Duplicating, editing, renaming, or other refactoring takes just moments. Deployment shrinks to activating an ABAP class, so we can create many apps quickly. For example, we built all the apps in the sample section fast, mostly by copy and paste — which would have been impractical for separately built and deployed frontend apps. This is a major reduction in complexity and an advantage of all 'Over the Wire' apps, as we noted in (3).

##### 19. No Extra Layer

Another way to reduce complexity: avoid creating extra customizing layers. As shown in (13), only one stack frame sits between the user's app and the HTTP-Handler, and no extra layers such as OData, SADL, or Gateway exist. This keeps the UI5 frontend framework and its functionality as pure as possible in the abap2UI5 apps on the backend.
...
Luckily, utility classes markedly simplify the creation process. For instance, `z2ui5_cl_xml_view` provides a class-based approach to create views with access to the UI5 API through ADT code completion:
...
To recap what we've covered so far: abap2UI5 is built in a highly generic way, placing most of the responsibility on the user's apps. As a result, we gain considerable flexibility and freedom in the app implementation, but we also take on full responsibility for view creation and program flow. We also have to keep these downsides in mind.
...
Also, using HANA DB capabilities directly on the frontend causes problems. Since every app uses the same generic HTTP-Service, we've decoupled the UI from the rest. RAP, by contrast, uses a typed OData and can tap into HANA capabilities through a CDS View directly (and skip the ABAP layer). This makes it easy to add pagination or fuzzy searches to UI5 freestyle or RAP apps. An OData-Service calling a CDS View of HANA directly is very effective here.
...
With this approach, dependencies remain limited to APIs and functions that are both cloud-released and available in earlier NetWeaver releases. To handle this, abap2UI5 uses SAP dependencies only when strictly needed — for instance, in GUID creation:
...
As you can see, creating methods compatible with both 'ABAP Cloud' and 'Standard ABAP' is much more complex. Fortunately, abap2UI5 requires only GUIDs as a dependency. But when building apps, keep this in mind (and practical feasibility remains to be seen). It does have one key advantage: abap2UI5 runs on ABAP 2305 and remains portable down to NetWeaver 7.02.
...
Automated ABAP downporting markedly improves efficiency. See the abaplint dashboard of this project and the abaplint tool. Beyond abaplint, this framework uses abapGit and benefits from the work of the people who built it:
...
Long article short: inspired by "HTML Over the Wire" (1)(2)(3), we mixed UI and Data together (7) and created a "UI5 Over the Wire" approach by sending the XML-View from the server (6). We then used a single generic HTTP-Service for all apps (13), independent of the View and Data Model (12). This gives us great flexibility, so we can create Data Models (10) and Views (11) dynamically at runtime, which markedly reduces the number of backend artifacts.
