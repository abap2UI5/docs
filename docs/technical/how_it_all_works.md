...
As we saw in (22), we can make apps very complex, but the opposite also works — we can make them very simple. One beautifully minimal approach uses `if_oo_adt_classrun`. By implementing a single method, we generate output with one click (`F9`). This is highly efficient and inspired abap2UI5. A comparison of both approaches:
