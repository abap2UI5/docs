...
This lets us define models not only at design time, but also at runtime. The user needs no extra work — abap2UI5 handles the entire process in the background on every AJAX request:
...
As a result, the View and Model remain independent of the HTTP-Service, and we're no longer forced to deliver a predefined static OData-Service for every app, as RAP does. This greatly reduces the number of backend artifacts:
...
Drafts also let us jump back to previous states with little effort, reducing the complexity we would usually encounter in an HDA context when implementing a cancel or exit event of a view. Like the HTTP-Service, these drafts are generic, removing the need to create typed draft tables for every data model manually, as RAP requires, and further reducing the number of backend artifacts:
