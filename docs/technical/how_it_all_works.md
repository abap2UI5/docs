...
Despite still relying on frontend HTML rendering, the app now retrieves all the needed information (view & data) over AJAX from the backend. The UI5 app remains a SPA, but its role shrinks to that of an HDA — responsible only for displaying the view and its data:
...
Drafts also let us jump back to previous states with minimal effort, reducing the complexity we would usually encounter in an HDA context when implementing a cancel or exit event of a view. Like the HTTP-Service, these drafts are generic, removing the need to create typed draft tables for every data model manually, as RAP requires, and further reducing the number of backend artifacts:
...
Most notably, compared to UI5, we can't implement offline capabilities — in such cases, we can't continually ask the server after every event to determine what happens next.
...
Also, Fiori Elements with all its floorplans & templates is straightforward and will receive many updates in the future. Ultimately, the wide range of UI5 use cases makes comparing the approaches difficult — and we can't fully discuss them here. Let's turn to the framework's codebase in the final part of this article.
...
As you can see, creating methods compatible with both 'ABAP Cloud' and 'Standard ABAP' is much more complex. Fortunately, abap2UI5 requires only GUIDs as a dependency. But when building apps, you must stay aware of this (and practical feasibility remains to be seen). It does have one key advantage: abap2UI5 runs on ABAP 2305 and remains portable down to NetWeaver 7.02.
