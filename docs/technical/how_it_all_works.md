...
This gives us a stateful-like PAI/PBO feel similar to SAP GUI apps, even though we're still working within the AJAX roundtrip logic. And since every request can land on a different application server, abap2UI5 is compatible with scalable cloud environments, ready for future compatibility:
