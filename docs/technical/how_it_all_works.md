...
Despite still relying on frontend HTML rendering, the app now fetches all the needed information (view and data) over AJAX from the backend. The UI5 app remains a SPA, but its role shrinks to that of an HDA — responsible only for displaying the view and its data:
