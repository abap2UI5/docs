...
The concept of **"HTML Over the Wire"** inspired one of the core ideas behind abap2UI5. This approach renders HTML directly on the server and sends it to the browser — without relying on JSON, client-side MVC frameworks, bundling, or transpiling pipelines.
...
Despite still relying on frontend HTML rendering, the app now fetches everything it needs (view and data) over AJAX from the backend. The UI5 app remains a SPA, but its role shrinks to that of an HDA — responsible only for displaying the view and its data:
...
As a result, the View and Model remain independent of the HTTP-Service, and we're no longer forced to ship a predefined static OData-Service for every app, as RAP does. This greatly reduces the number of backend artifacts:
...
Since all user apps are also in pure ABAP, we can now maintain and build everything in the backend. Duplicating, editing, renaming, or other refactoring takes just moments. Deployment shrinks to activating an ABAP class, so we can create many apps quickly. For example, we built all the apps in the sample section fast, mostly by copy and paste — which would have been impractical for separately built and deployed frontend apps. This is a big reduction in complexity and an advantage of all 'Over the Wire' apps, as we noted in (3).

##### 19. No Extra Layer

Another way to reduce complexity: avoid creating extra customizing layers. As shown in (13), only one stack frame sits between the user's app and the HTTP-Handler, and no extra layers such as OData, SADL, or Gateway exist. This keeps the UI5 frontend framework and its functionality intact in the abap2UI5 apps on the backend.

UI5 evolves quickly, and extra layers can become outdated fast. With this approach, all future UI5 controls work in abap2UI5 automatically. But there's a tradeoff: you have to face the complexity of the frontend UI5 API and learn the concepts of XML views and UI5 controls. Ultimately, it's a matter of preference: learn UI annotations, or learn the concepts of SAP UI5 directly.
...
This contrasts with RAP, where you benefit from well-documented and organized extra layers, though sometimes they have limited functionality. Take side effects, for example: RAP restricts you to the `+`, `-`, and `*` operators. In abap2UI5, you write JavaScript directly, which takes much more knowledge, but in return you get access to full expression binding on the frontend:
...
As we saw in (22), we can make apps very complex, but the opposite also works — we can make them very simple. One elegantly minimal approach uses `if_oo_adt_classrun`. By implementing a single method, we generate output with one click (`F9`). This is highly efficient and inspired abap2UI5. A comparison of both approaches:
...
Most notably, compared to UI5, we can't implement offline capabilities — in those cases, we can't continually ask the server after every event to determine what happens next.
...
And Fiori Elements, with all its floorplans and templates, is straightforward and will keep receiving updates. Ultimately, the wide range of UI5 use cases makes comparing the approaches difficult — and we can't fully discuss them here. Let's turn to the framework's codebase in the final part of this article.
...
Automated ABAP downporting greatly improves efficiency. See the abaplint dashboard of this project and the abaplint tool. Beyond abaplint, this framework uses abapGit and benefits from the work of the people who built it:
...
Next, we explored several ideas for how the framework reduces its own complexity by avoiding frontend artifacts (17), removing extra customizing layers (19), and separating the view from the framework (21), as well as app-specific JS or HTML (22). The result is a pure source-code approach with only one database table, two interfaces, one class, and just 2,300 lines of code (25). We build it in a single code line (27), making it cloud- and on-premise-ready and downportable to old releases (28). Together with abapGit, you can build apps that run on nearly every release.
