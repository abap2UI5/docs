...
Drafts also let us jump back to previous states with minimal effort, reducing the complexity we would typically encounter in an HDA context when implementing a cancel or exit event of a view. Like the HTTP-Service, these drafts are generic, removing the need to create typed draft tables for every data model manually, as RAP requires, and further reducing the number of backend artifacts:
