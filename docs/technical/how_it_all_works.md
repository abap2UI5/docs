...
Since all user apps are also in pure ABAP, we can now maintain and build everything in the backend. Duplicating, editing, renaming, or other refactoring takes just moments. Deployment shrinks to activating an ABAP class, so we can create many apps quickly. For example, we built all the apps in the samples section fast, mostly by copy and paste — which would have been unrealistic for separately built and deployed frontend apps. This is a big reduction in complexity and an advantage of all 'Over the Wire' apps, as we noted in (3).

##### 19. No Extra Layer

Another way to reduce complexity: avoid creating extra customizing layers. As shown in (13), only one stack frame sits between the user's app and the HTTP-Handler, and no extra layers like OData, SADL, or Gateway exist. This keeps the UI5 frontend framework and its functionality intact in the abap2UI5 apps on the backend.

UI5 evolves quickly, and extra layers can become outdated quickly. With this approach, all future UI5 controls work in abap2UI5 automatically. But there's a tradeoff: you have to face the complexity of the frontend UI5 API and learn the concepts of XML views and UI5 controls. Ultimately, it's a matter of preference: learn UI annotations, or learn the concepts of SAP UI5 directly.

##### 20. No Hiding of Complexity

But having no extra layer also means the framework doesn't always abstract complexity away — unlike what other frameworks aim for. In abap2UI5, you send your XML-View directly to the frontend, and you're responsible for ensuring it's valid and executable:
...
Luckily, utility classes greatly simplify the creation process. For example, `z2ui5_cl_xml_view` provides a class-based approach to create views with access to the UI5 API through ADT code completion:
...
This contrasts with RAP, where you benefit from well-documented and organized extra layers, though sometimes they have limited functionality. Take side effects, for example: RAP limits you to the `+`, `-`, and `*` operators. In abap2UI5, you write JavaScript directly, which demands more knowledge, but in return you get access to full expression binding on the frontend:
...
In the current approach, we separate them from the view and create them through extra methods. The framework then handles data binding and transfer automatically (9):
...
As we saw in (22), we can make apps very complex, but the opposite also works — we can make them very simple. One minimal yet elegant approach uses `if_oo_adt_classrun`. By implementing a single method, we generate output with one click (`F9`). This is highly efficient and inspired abap2UI5. A comparison of both approaches:
...
To recap what we've covered so far: abap2UI5 is built in a highly generic way, placing most of the responsibility on the user's apps. As a result, we gain a lot of flexibility and freedom in the app implementation, but we also take on full responsibility for view creation and program flow. We also have to keep these tradeoffs in mind.

##### 24. Downsides Compared to UI5 & RAP

Most notably, compared to UI5, we can't implement offline capabilities — in those cases, we can't keep asking the server after every event to determine what happens next.
