...
- **`client->view_model_update( )`** — sends only updated model data. UI5 refreshes the existing view through data binding, re-rendering only the changed controls. This keeps UI state (scroll position, focus, etc.) and is much faster.
