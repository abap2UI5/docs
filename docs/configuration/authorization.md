...
abap2UI5 gives you flexibility in managing authorization. It has no built-in authorization mechanism, so you can build your own solution at either the service or the application level.
...
You can also pair this with SAP authorization objects. The example below uses a custom authorization object `Z_APP_AUTH` with an `APP` field — create the object in transaction `SU21` and assign it to the relevant roles on your system:
```abap
CLASS z2ui5_cl_launchpad_handler DEFINITION PUBLIC.
