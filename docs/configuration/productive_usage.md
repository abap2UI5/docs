...
#### Stable Version
The project is continuously developed, so there's no specific "stable" version. However, changes to the public APIs are kept to a minimum to avoid frequent refactoring of apps. You can use the [releases](https://github.com/abap2UI5/abap2UI5/releases/) instead of pulling the main branch and update from time to time to reduce the effort needed for refactoring.
