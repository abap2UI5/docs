...
### S/4 Public Cloud
In `S/4 Public Cloud`, the HTTP endpoint needs `S_DEVELOP` authorization, which makes it unfit for production. To make the app reachable for business users, install a separate frontend app and create an extra tile. See [S/4 Public Cloud](/configuration/s4_public_cloud) for details.
