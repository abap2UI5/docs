---
outline: [2, 4]
---
# Contact

abap2UI5 is an open-source project that values transparent, publicly visible communication. Whenever possible, use public channels so others can join the conversation and benefit from it. Reach out through these platforms:

- Repository: Open an issue on [GitHub](https://github.com/abap2UI5/abap2UI5/issues)
- Slack: Join the abap2UI5 channel on [abapGit Slack](https://communityinviter.com/apps/abapgit/abap)
- Social Media: Share your thoughts on [LinkedIn](https://www.linkedin.com/company/abap2ui5)

For private inquiries, email <contact@abap2UI5.org>. Since this is a spare-time project, responses may take a while.
