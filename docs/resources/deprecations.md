...
Things in abap2UI5 that have a successor, and the ones that are already gone.
Every entry says what to write instead, with the old and the new code next to
each other.

Two categories, and the difference decides how urgent an entry is:

- **[Obsolete](#obsolete-still-compiles)** — still compiles, still ships. Some
  of these still work, some are empty methods that do nothing. Nothing breaks
  today; migrate when you next touch the app.
- **[Removed](#removed-the-build-breaks)** — gone. The build breaks until you
  change the call.
...
## Removed: the build breaks

### `z2ui5_if_app~check_sticky` / `check_initialized` <br> *next release*

Both attributes are gone from `z2ui5_if_app`, which now carries only `id_draft`,
`id_app` and `main( )`. They were framework bookkeeping that every app could
write, and a write was never honoured the way it read.

```abap
" old
IF me->z2ui5_if_app~check_initialized = abap_false.
  me->z2ui5_if_app~check_initialized = abap_true.
  init_data( ).
ENDIF.
me->z2ui5_if_app~check_sticky = abap_true.
...
`id_draft` and `id_app` stay and are **not** obsolete: `id_draft` is the handle
`db_load_by_app( )` resolves an app reference by.

### `client->set_nav_back( )` / `set_nav_routing( )` <br> *next release*

Both are follow-up-action events now.
...
The routing mode decides how much of the running app the URL hash carries:
`cs_nav_mode-default` leaves the hash untouched, `-fresh` writes
`#/app/<CLASS>` and restarts the app clean on Back/reload, `-keep` writes
`#/app/<CLASS>/<DRAFT>` and restores the preserved state.

`set_push_state( )` and `set_app_state_active( )` **stay** as methods and
delegate to their events — no change needed there.

### `cs_event-nav_to_route` <br> *next release*

It navigated to another app by writing the hash and letting the router start the
target — a second, frontend-side way into a navigation the backend already owns.

```abap
" old
client->follow_up_action( val   = client->cs_event-nav_to_route
                          t_arg = VALUE #( ( `ZCL_MY_OTHER_APP` ) ) ).

" new
client->nav_app_call( NEW zcl_my_other_app( ) ).
```

`nav_app_call( )` is the real navigation: the target app is instantiated on the
server and the calling app is preserved on the stack. With routing switched on
it pushes the very same route history entry, so Back keeps working as before.

### `cs_event-history_back` <br> *next release*

A one-line wrapper around `history.back()` from the era before
`follow_up_action( )` took raw JavaScript.

```abap
" old
client->follow_up_action( client->cs_event-history_back ).

" new - inside the app stack, the usual case
client->nav_app_leave( ).

" new - if you really mean the browser history
client->follow_up_action( |history.back()| ).
```

`nav_app_leave( )` returns to the calling app through the app stack instead of
stepping the browser history — that is what most apps meant.

### `client->get( )-viewname` <br> *next release*

The framework never filled this component, so the read always returned an empty
string. Reading it no longer compiles — delete the read.

### The curated formatter loses five helpers <br> *next release*

`round2DP`, `dimensions`, `stockStatusState`, `stockStatusIcon` and
`deliveryStatusState` are gone from `z2ui5/model/formatter` (`z2ui5.Formatter`).
abap2UI5 is a thin frontend: rounding a number, joining fields and mapping a
business status to a `ValueState` or an icon are things ABAP finishes.

```abap
" old - the view called the formatter
|\{ path: `{ client->_bind( val = mv_status path = abap_true ) }`,
    formatter: 'Formatter.stockStatusState' \}|

" new - ABAP computes it, the view binds the result
ms_data-status_state = SWITCH #( ms_data-stock
                                 WHEN 0 THEN `Error`
                                 WHEN 1 THEN `Warning`
                                 ELSE        `Success` ).
...
)->a( n = `state` v = client->_bind( ms_data-status_state ) )
```

Unchanged and staying: the date helpers (`DateCreateObject`,
`DateAbapDateToDateObject`, `DateAbapDateTimeToDateObject`) and
`expandInlineIcons` — a JS `Date` and an icon-font glyph are the two things the
backend physically cannot produce.
