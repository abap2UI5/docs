...
- Hosting outside an SAP system: seams for the draft store and the environment, and the transpiled framework is packaged for npm (published as `@abap2ui5/node` from the next release on); every release carries the backend already built (`backend-<version>.tar.gz`)
