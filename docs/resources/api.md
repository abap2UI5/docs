...
Generated from [`z2ui5_if_client`](https://github.com/abap2UI5/abap2UI5/blob/main/src/02/z2ui5_if_client.intf.abap)
on **main** — 42 methods, 4 constant
...
The one difference is follow_up_action( )'s leading CASE, which claims cs_event-hash_routing / hash_set / hash_replace / hash_attach_changed / app_state_set_active (and their obsolete set_* spellings) before that branch. Those are backend-side navigation options rather than frontend handlers, so wiring one into a view attribute never dispatched anything here either.
...
The app-state hash: while active, the URL carries the id of the CURRENT app state (`#/z2ui5-xapp-state=<id>`), advanced on every roundtrip - a reload, a bookmark or a shared link restores the exact state (the draft the framework persists anyway is the state container, nothing extra is stored). abap_false switches the URL tracking off again. Mutually exclusive with hash_routing and hash_attach_changed - each claims the whole app hash.
...
The absolute URL of the CURRENT app state - the link that restores exactly this roundtrip's state, FLP-safe: the shell hash of the page survives in the link, so the recipient lands in this app instead of on the launchpad home page. Composed from the browser's own location (origin/pathname/search ride with the requests) plus this response's draft id. The app owns the string: copy it with cs_event-clipboard_copy, show it in an Input, mail it, render it as a QR code. Supersedes the obsolete cs_event-clipboard_app_state.

Returns `string`.

### `hash_set`

HashChanger#setHash: write VAL as the app's URL hash - a PUSHED history entry, so the browser Back button has a step to take. The 1:1 counterpart of a UI5 router's navTo. With cs_event-hash_attach_changed registered the value is the WHOLE app hash (`/Page2`); without a listener the legacy suffix behavior applies (see the obsolete set_push_state, which this renames).
...
HashChanger#replaceHash: write VAL as the app's URL hash WITHOUT a new history entry - the UI5 router's navTo( ..., true ). What FlexibleColumnLayout apps do when a NAVIGATION ARROW changes the layout: the URL follows, but Back does not step through arrow drags.

| Parameter | Type | Default | Description |
|---|---|---|---|
| `val` | `string` | *optional* |  |

### `set_app_state_active` <Badge type="warning" text="obsolete" />

obsolete spelling of app_state_set_active( ) - same behavior. It stays in the interface so existing apps keep compiling.

| Parameter | Type | Default | Description |
|---|---|---|---|
| `val` | `abap_bool` | `abap_true` |  |

### `set_push_state` <Badge type="warning" text="obsolete" />

obsolete spelling of hash_set( ) - same behavior ('push state' described the old history.pushState implementation, which the HashChanger-backed write replaced). It stays in the interface so existing apps keep compiling.
...
| `cs_event-hash_set` | `SET_PUSH_STATE` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-hash_replace` | `HASH_REPLACE` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-hash_back` | `HASH_BACK` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-hash_attach_changed` | `HASH_ATTACH_CHANGED` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-hash_routing` | `SET_NAV_ROUTING` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-app_state_set_active` | `SET_APP_STATE_ACTIVE` | *with their obsolete spellings below - both names reach the same branch* |
| `cs_event-set_app_state_active` | `SET_APP_STATE_ACTIVE` | *obsolet* |
| `cs_event-set_push_state` | `SET_PUSH_STATE` | *obsolet* |
| `cs_event-set_nav_routing` | `SET_NAV_ROUTING` | *obsolet* |
| `cs_event-clipboard_app_state` | `CLIPBOARD_APP_STATE` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-image_editor_popup_close` | `IMAGE_EDITOR_POPUP_CLOSE` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-nav_container_to` | `NAV_CONTAINER_TO` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-nest_nav_container_to` | `NEST_NAV_CONTAINER_TO` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-nest2_nav_container_to` | `NEST2_NAV_CONTAINER_TO` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-popup_nav_container_to` | `POPUP_NAV_CONTAINER_TO` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-popover_nav_container_to` | `POPOVER_NAV_CONTAINER_TO` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-z2ui5` | `Z2UI5` | *the live hash ride with the requests), so the app can also SHOW it* |
| `cs_event-wizard_set_next_step` | `WIZARD_SET_NEXT_STEP` | *the live hash ride with the requests), so the app can also SHOW it* |
...
Reading the trace alone pushes no model, so the browser goes on showing the refused text until the app writes something. See z2ui5_cl_ui5_srv_model=>delta_apply_field, which fills it.
