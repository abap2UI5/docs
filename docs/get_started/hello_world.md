...
ENDINTERFACE.
```
The `client` object is your only entry point to the framework. Use it to display views, handle events, exchange data, and navigate between apps.
...
### Data Exchange
Finally, add a public attribute so you can send data to the backend:
```abap
CLASS zcl_app_hello_world DEFINITION PUBLIC.
  PUBLIC SECTION.
    INTERFACES z2ui5_if_app.
...
While the HTTP handler must distinguish between Standard ABAP and ABAP for Cloud, the apps themselves are independent. You're free to decide whether to build your apps with ABAP Cloud compatibility.
:::
