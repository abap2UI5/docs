...
- **Look a pattern up.** The [Cookbook](/cookbook/cheat_sheet) is the reference
  for everyday work — views, events, binding, tables, popups, navigation.
- **Set up the tools.** Optional, and worth the ten minutes: the
  [tooling](/advanced/tooling) around the framework, and
  [Developing with AI](/get_started/ai) if an assistant writes some of it.
- **Go live.** Security, performance, the launchpad and the transport are under
  [Configuration](/configuration/installation); [Productive Usage](/configuration/productive_usage)
  is the page to read before the first go-live.
- **See who runs it.** [Who Uses abap2UI5?](/resources/who_uses) lists ready-to-use
  apps, real-world scenarios and companies already running on it; the
  [add-ons](/resources/addons) cover what not every app needs.
