...
### About
Since launching in 2023, abap2UI5 has grown from a small side project into a community-driven framework used by ABAP developers worldwide. The framework handles all the frontend complexity, letting you focus on business logic with your existing ABAP skills.
...
#### Architecture
abap2UI5 takes a "thin frontend" approach — all processing, logic, and data management remain in the backend. This design simplifies configuration, reduces client-side complexity (no more cache-clearing hassles), and keeps business logic and sensitive data securely on the server.
...
#### ABAP Cloud
abap2UI5 uses only released APIs, making it a strong fit for on-stack and side-by-side extensions in ABAP for Cloud. Use modern ABAP syntax features like CDS, ABAP SQL, and EML inside your apps.

#### Clean Core
By relying only on released APIs, abap2UI5 keeps your applications "cloud-ready" and "upgrade-stable," aligned with SAP's clean-core principles. Your investment in abap2UI5 apps remains safe across future SAP system upgrades.
