...
Two places to run an abap2UI5 app, and — this is the part people usually get
backwards — the choice is about lifecycles rather than about code. The class is
...
maintenance slots, its change freeze in December.

![abap2UI5 and the apps run inside S/4HANA and use released APIs only](/advanced/use_cases/on_stack_level_a.svg){ width=90% }
...
The cost is equally plain, and it is not only technical: a remote call is
slower than a local read, the released remote APIs are narrower than what a
local `SELECT` can reach, and there is a second system to run, monitor and pay
for. When the released remote APIs do not cover the case, a service on the S/4
side closes the gap and is graded on its own — the BTP side stays Level A:
