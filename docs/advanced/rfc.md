...
The [abap2UI5 RFC Connector](https://github.com/abap2UI5-addons/rfc-connector) lets you trigger abap2UI5 apps remotely via RFC between two ABAP systems. This helps in distributed system landscapes or with legacy systems that can't directly host UI5 frontend logic.

#### When to use
The RFC Connector unlocks several useful scenarios:
- Frontend/Backend Split: abap2UI5 apps live in the backend, but users only have access to the frontend server
- Legacy Systems (e.g., ECC): you can downport abap2UI5 onto older systems and then trigger it from a newer system via RFC
...
_Prerequisite: Set up a destination in SM59 so you can call the source system via RFC. Install abap2UI5 on both systems._
