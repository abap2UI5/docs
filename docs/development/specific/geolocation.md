...
abap2UI5 provides a custom control for retrieving geolocation data from the user's device — longitude, latitude, altitude, and speed. This helps with logistics apps, field service tools, or any case where location matters.
...
Geolocation requires the user to grant permission in the browser. Serve your HTTP endpoint over HTTPS, since most browsers block geolocation on insecure origins.
:::
