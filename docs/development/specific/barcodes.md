...
Since UI5 version 1.102, the `sap.ndc.BarcodeScannerButton` control is part of the UI5 library, making barcode scanning straightforward. You can use the barcode scanner control like any other UI5 control with abap2UI5. Below is an example demonstrating basic functionality. You can customize the handling after the scanning event fires:
...
#### Focus Handling
Most scanner devices emulate a keyboard. In such cases, you can create an input field and set focus correctly. The scanned data will populate the input field as if typed on a keyboard.
...
Providing audio feedback can help in specific contexts. Below is an example that plays a sound if a user fails to scan a value. The sound is a .wav file in the SAP MIME repository at `/SAP/PUBLIC/BC/ABAP/mime_demo/bam.wav`:
...
#### Display Barcodes
If you also need to display barcodes, you can use bwip-js, which is available as part of the js-libraries add-on. See [Add-ons](/resources/addons) for more details.
