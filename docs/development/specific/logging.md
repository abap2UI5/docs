...
Logging is critical for business processes. In ABAP systems, the standard tool is the Business Application Log (BAL), available both on-premise and in ABAP Cloud environments. With abap2UI5, use BAL like you would in classic ABAP and display logs with the framework's built-in popups.
...
#### abap-logger
You can also use the open-source project [**abap-logger**](https://github.com/ABAP-Logger/ABAP-Logger). It simplifies work with BAL logs and pairs smoothly with abap2UI5:
```abap
METHOD z2ui5_if_app~main.
...
#### BAL Popup
Compared to message classes, BAL logs carry more detail — like timestamps. Use the dedicated BAL log popup to display them. All examples above work with the `Z2UI5_CL_POP_BAL` popup for richer output. An example with abap-logger:
