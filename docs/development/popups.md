...
UI5 offers popups and popovers that overlay specific areas of the view. This section shows how to build them in abap2UI5.
...
#### Flow Logic
A common popup flow shows a normal view, opens a popup, and finally closes it. Structure it like this:
```abap
  METHOD z2ui5_if_app~main.
...
To manage multiple stacked popups, note that abap2UI5 shows only one popup at a time on the frontend. But you can maintain a popup stack in your backend logic and re-display the previous popup as needed. See `Z2UI5_CL_DEMO_APP_161`.

### Popover
To display a popover, call `client->popover_display` and pass the ID of the control where the popover should appear:
```abap
  METHOD z2ui5_if_app~main.
...
### Built-in Popups
A few pre-built popup classes cover common cases:
...
Help grow this collection to cover more use cases — contributions are welcome.
