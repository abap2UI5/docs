...
By default, you bind public attributes of your class to UI5 properties through `_bind` and `_bind_edit`. For cases that require access to large datasets, you can also use existing OData services. OData provides features like pagination and growing that improve performance when handling large amounts of data.

#### Define Additional Model
As an example, we use the test OData service `/sap/opu/odata/DMO/UI_FLIGHT_R_V2/`, available in most ABAP systems. Ensure the service is publicly accessible. The method below defines the model and exposes it under the name `FLIGHT`:
```abap
...
client->follow_up_action( client->_event_client(
    ...
    t_arg = VALUE #(
        ...
        ( `TRAVEL` ) ) ) ).
```
For a working code snippet, see the sample `Z2UI5_CL_DEMO_APP_315`.
...
```
Use these SAP annotations in the UI5 view to reuse backend translations through the `label` property. The metadata binding path follows this pattern:
