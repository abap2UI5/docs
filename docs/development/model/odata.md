...
As an example, we use the test OData service `/sap/opu/odata/DMO/UI_FLIGHT_R_V2/`, which is available in most ABAP systems. Ensure the service is publicly accessible. Use the following method to define the model and make it available under the name `FLIGHT`:
```abap
...
```
The `growing` property ensures data is loaded in batches rather than all at once, improving performance.
...
```
We can use these SAP annotations in our UI5 view to reuse backend translations via the property `label`. The metadata binding path follows this pattern:
